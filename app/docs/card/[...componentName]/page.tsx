...
const CardDocumentation = () => {
  ...
  useEffect(() => {
    const getFileContent = async () => {
      ...
      try {
        const content = await fetchFileContent(`card/${componentName}.tsx`);
  ...
  useEffect(() => {
    if (COMPONENT_CONFIG["card"]?.[componentName] as ComponentConfigType) {
      setComponentConfig(COMPONENT_CONFIG["card"]?.[componentName]);
    } else {
      redirect("/");
    }
  }, []);
