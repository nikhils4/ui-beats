...
```tsx
<SubscribeButton text="Follow Updates" className="saas-button shadow-lg" />
```

### Community Platforms

```tsx
<SubscribeButton text="Join Community" onClick={() => initiateMembership()} />
```
...
function App() {
  return (
    <ThemeProvider attribute="class">
      <SubscribeButton onClick={() => console.log("New subscriber!")} />
    </ThemeProvider>
  );
}
```
